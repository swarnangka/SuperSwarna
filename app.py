"""
Parabolic Trends v7
Tasks: remove 4H HHLL, price % change on 1H cards, dashboard button,
parallel fetches, market hours detection, HHLL load-on-demand,
stale-while-revalidate badge, sector pre-cache, loading UX, mobile layout.
"""
...
from data_layer import INDEX_TOKENS, connected, fetch_yf_parallel, ist_now, is_market_hours, market_status
from engine import index_risk
from chartink import run_chartink_scan, DEFAULT_SCREENERS, fetch_tape_scan, TAPE_SCANS
from market_web import gainers_losers, nifty50, advance_decline, nifty50_heatmap_yf, sector_heatmap_yf
...
.t-item{display:inline-flex;align-items:center;gap:5px;padding:0 16px;
...
.mkt-badge{font-size:11px;padding:3px 10px;border-radius:999px;font-weight:600;}
.mkt-live{background:rgba(46,196,160,.12);color:var(--green);}
.mkt-closed{background:rgba(72,79,88,.2);color:var(--txt3);}
.mkt-pre{background:rgba(210,153,34,.12);color:var(--amber);}
...
# ── Tape helpers ───────────────────────────────────────────────────────────────
def build_tape_html(items: list) -> str:
    seg = ""
    for it in items:
        chg = it.get("chg", 0)
        arrow = "▲" if chg >= 0 else "▼"
        ...
        seg  += (f'<span class="t-item">'
                 ...
                 f'</span>')
    return f'<div class="tape-inner fast">{seg}{seg}</div>' if seg else ""

def build_scan_tape_html(items: list) -> str:
    if not items:
    ...
    for it in items:
        ...
        seg += (f'<span class="t-item">'
                ...
                f'</span>')
    return f'<div class="tape-inner med">{seg}{seg}</div>' if seg else ""
...
mkt_cls = ("mkt-live" if mkt == "Live" else
           "mkt-pre"  if mkt == "Pre-market" else "mkt-closed")
mkt_html = f'<span class="mkt-badge {mkt_cls}">{mkt}</span>'
...
with h1:
    st.markdown(
        ...
        f'<div class="hdr-right">{conn_html}{mkt_html}'
        f'<span class="clock">{ist_now().strftime("%d %b %Y · %H:%M")} IST</span>'
        f'</div></div>', unsafe_allow_html=True)
with h2:
    st.markdown('<div style="padding-top:8px" class="dash-btn">',
                unsafe_allow_html=True)
    st.link_button("📊 Dashboard",
                   "https://trading-journal-ryxzgf6acavnzc6mv6vdzn.streamlit.app/")
    st.markdown('</div>', unsafe_allow_html=True)

# ── Market closed banner ───────────────────────────────────────────────────────
if mkt != "Live":
    st.markdown(
        f'<div class="mkt-closed-banner">Markets {mkt} · '
        f'Showing last session data · Next open 09:15 IST</div>',
        unsafe_allow_html=True)

# ══ TAPES ════════════════════════════════════════════════════════════════════
g_items = global_ticker_data()
g_html  = build_tape_html(g_items)
st.markdown(f'<div class="tape-wrap" style="background:#0A0F16;padding:5px 0">'
            f'{g_html}</div>', unsafe_allow_html=True)

BADGE_COLORS = {"52WH":("#7C5CFC","#fff"), "MM":("#2EC4A0","#0D1117")}
for tape_label in ["52WH","MM"]:
    tape_items = fetch_tape_scan(tape_label)
    tape_html  = build_scan_tape_html(tape_items)
    bg, fg = BADGE_COLORS.get(tape_label, ("#484F58","#fff"))
    ts = ist_now().strftime("%H:%M")
    st.markdown(
        f'<div style="display:flex;align-items:center;'
        f'border-bottom:1px solid var(--border);background:var(--bg);padding:3px 0">'
        f'<span style="background:{bg};color:{fg};flex-shrink:0;margin:0 8px;'
        f'font-size:10px;font-weight:700;letter-spacing:1px;padding:2px 8px;'
        f'border-radius:4px;font-family:monospace">{tape_label}</span>'
        f'<div style="overflow:hidden;flex:1"><div style="display:flex">{tape_html}</div></div>'
        f'<span style="font-size:10px;color:var(--txt3);margin:0 8px;'
        f'font-family:monospace;flex-shrink:0">{ts}</span>'
...
# ══ HHLL COMMODITY SIGNALS — load on demand ══════════════════════════════════
st.markdown('<div class="sec"><span class="sec-title">Commodity Signals · HHLL(29) · USD</span>'
            '<span class="sec-line"></span></div>', unsafe_allow_html=True)

with st.expander("Commodity Signals", expanded=False):
    hl1, hl2 = st.columns([1,4])
    with hl1:
        load_hhll = st.button("Load", key="hhll_load")
    with hl2:
        if st.session_state.get("hhll_ts"):
            st.markdown(
                f'<span class="stale-badge stale-live">'
                f'Data as of {st.session_state["hhll_ts"]} · refreshes every 30 min</span>',
                unsafe_allow_html=True)
    if load_hhll:
        st.session_state["hhll_done"] = True
        st.session_state.pop("hhll_cache", None)

    if st.session_state.get("hhll_done"):
        if "hhll_cache" not in st.session_state:
            with st.spinner("Fetching commodity data…"):
                sigs = get_hhll_signals()
            st.session_state["hhll_cache"] = sigs
            st.session_state["hhll_ts"] = ist_now().strftime("%d %b · %H:%M IST")
        else:
            sigs = st.session_state["hhll_cache"]
        ...
        for ci, (name, data) in enumerate(sigs.items()):
            with hcols[ci]:
                ...
                # % change: current price vs signal price
                pchg = None
                ...
                pchg_html = ""
                if pchg is not None:
                    pcls = "t-up" if pchg >= 0 else "t-dn"
                    pchg_html = f'<span class="hl-pchg {pcls}">{pchg:+.2f}% since signal</span>'
                st.markdown(
f'<div class="hl-card">'
...
f'<div class="hl-price">{pstr}</div>{pchg_html}'
f'</div>'
...
f'</div>'
f'<div class="hl-sig-ts">Signal since: {sig_time} · {elapsed} {spstr}</div>'
f'</div>', unsafe_allow_html=True)
...
with st.expander("Market Internals"):
    ...
    if st.session_state.get("int_done"):
        ...
        with st.spinner("Loading… (parallel fetch)"):
            ...
            sec_heat = sector_heatmap_yf()
            gl       = gainers_losers()

        st.markdown(
            f'<span class="stale-badge stale-live" style="display:inline-block;margin-bottom:8px">'
            f'Data as of {data_ts}</span>', unsafe_allow_html=True)

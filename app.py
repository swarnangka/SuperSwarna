...
/* Mobile: hide scan tapes (they stack), keep only global tape */
@media(max-width:768px){
  .tape-scan-row{display:none!important;}
  .t-item{padding:0 9px!important;font-size:11px!important;}
  .t-name{font-size:10px!important;}
}
...
# Scan tapes — hidden on mobile via .tape-scan-row CSS class
...
for tape_label in ["52WH","MM"]:
    ...
    st.markdown(
        f'<div class="tape-scan-row" style="display:flex;align-items:center;'
...
# ══ HHLL COMMODITY SIGNALS — auto-loads on page open ═════════════════════════
...
f'</div>'
f'<div class="hl-sig-ts">Signal since: {sig_time} · {elapsed}'

...
with st.expander("Commodity Signals", expanded=False):
    ...
    with hl2:
        st.markdown(
            '<span class="stale-badge">Refreshes every 30 min when loaded</span>',
            unsafe_allow_html=True)
    if load_hhll:
    ...
    if st.session_state.get("hhll_done"):
        with st.spinner("Fetching commodity data…"):
            # Use @cache_data TTL only — no session_state cache so 30-min refresh works
